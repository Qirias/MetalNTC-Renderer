...
/// Constructs a symmetric orthographic projection matrix with reverse-Z, from left-hand eye
/// coordinates to left-hand clip coordinates.
/// That maps (left, top) to (-1, 1), (right, bottom) to (1, -1), and (nearZ, farZ) to (1, 0) for reverse-Z.
/// The first four arguments are signed eye coordinates.
/// nearZ and farZ are absolute distances from the eye to the near and far clip planes.
matrix_float4x4 AAPL_SIMD_OVERLOAD matrix_ortho_left_hand_reverse_z(float left, float right, float bottom, float top, float nearZ, float farZ);
